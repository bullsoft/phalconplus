...
PHP_METHOD(PhalconPlus_Bootstrap, __construct) {
	...
	ZEPHIR_CALL_METHOD(&config, this_ptr, "initconf", NULL, 82);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_1);
	object_init_ex(&_1, phalconplus_app_app_ce);
	ZEPHIR_CALL_METHOD(NULL, &_1, "__construct", NULL, 83, &config);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&_2, &_1, "boot", NULL, 84, &env, &runMode);
	zephir_check_call_status();
