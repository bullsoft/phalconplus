...
PHP_METHOD(PhalconPlus_Http_NonPsrRequest, __construct) {
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "mapfiles", NULL, 124, &_52);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(&uploads) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&uploads), _0)
		{
			...
			if (Z_TYPE_P(&file) == IS_ARRAY) {
				ZEPHIR_CALL_METHOD(NULL, this_ptr, "mapfiles", &_2, 124, &file);
				zephir_check_call_status();
			} else if (zephir_is_instance_of(&file, SL("Psr\\Http\\Message\\UploadedFileInterface"))) {
				ZEPHIR_CALL_FUNCTION(&_3$$5, "sys_get_temp_dir", &_4, 125);
				zephir_check_call_status();
				ZEPHIR_INIT_NVAR(&_5$$5);
				ZVAL_STRING(&_5$$5, "upload");
				ZEPHIR_CALL_FUNCTION(&tmpname$$5, "tempnam", &_6, 126, &_3$$5, &_5$$5);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				if (Z_TYPE_P(&file) == IS_ARRAY) {
					...
					ZEPHIR_CALL_FUNCTION(&_12$$9, "sys_get_temp_dir", &_4, 125);
					zephir_check_call_status();
					ZEPHIR_INIT_NVAR(&_13$$9);
					ZVAL_STRING(&_13$$9, "upload");
					ZEPHIR_CALL_FUNCTION(&tmpname$$9, "tempnam", &_6, 126, &_12$$9, &_13$$9);
					zephir_check_call_status();
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_0), _1)
		{
			...
			if (zephir_array_isset_string_fetch(&tmp, &item, SL("tmp_name"), 0)) {
				if ((zephir_file_exists(&tmp) == SUCCESS)) {
					ZEPHIR_CALL_FUNCTION(NULL, "unlink", &_3, 127, &tmp);
					zephir_check_call_status();
				}
			}
		} ZEND_HASH_FOREACH_END();
