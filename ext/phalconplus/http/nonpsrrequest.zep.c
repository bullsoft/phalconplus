	...
	if (Z_TYPE_P(&_24) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_24), _27, _28, _25)
		{
			...
			if (Z_TYPE_P(&v) == IS_ARRAY) {
				ZEPHIR_MAKE_REF(&v);
				ZEPHIR_CALL_FUNCTION(&_32$$23, "reset", &_33, 67, &v);
				ZEPHIR_UNREF(&v);
				zephir_check_call_status();
			} else {
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (Z_TYPE_P(&v) == IS_ARRAY) {
					ZEPHIR_MAKE_REF(&v);
					ZEPHIR_CALL_FUNCTION(&_38$$24, "reset", &_33, 67, &v);
					ZEPHIR_UNREF(&v);
					zephir_check_call_status();
				} else {
