...
PHP_METHOD(PhalconPlus_Rpc_Yar, __construct) {
	...
	if (ZEPHIR_IS_EQUAL(&_2, &formater)) {
		...
		ZEPHIR_CALL_FUNCTION(&_4$$4, "msgpack_unpack", NULL, 129, &rawBody);
		zephir_check_call_status();
...
PHP_METHOD(PhalconPlus_Rpc_Yar, handle) {
	...
	if (zephir_is_true(&_4)) {
		...
		ZEPHIR_CALL_FUNCTION(&_8$$3, "highlight_string", &_9, 130, &sampleCodes, &__$true);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&_13$$3, "highlight_string", &_9, 130, &expectedRet, &__$true);
		zephir_check_call_status();
		...
		/* try_start_1: */
			...
			if (ZEPHIR_IS_EMPTY(&_18$$5)) {
				...
				ZEPHIR_CALL_METHOD(NULL, &_19$$6, "__construct", NULL, 20, &_20$$6);
		...
		if (EG(exception)) {
			...
			if (zephir_instance_of_ev(&_17$$4, zend_exception_get_default(TSRMLS_C))) {
				...
				zephir_check_call_status();
				ZVAL_LONG(&_29$$8, 1);
				ZEPHIR_CALL_FUNCTION(&_30$$8, "max", NULL, 131, &_28$$8, &_29$$8);
				zephir_check_call_status();
