...
{
	ZEPHIR_REGISTER_CLASS_EX(PhalconPlus\\Rpc\\Client\\Adapter, Simple, phalconplus, rpc_client_adapter_simple, phalconplus_rpc_client_abstractclient_ce, phalconplus_rpc_client_adapter_simple_method_entry, 0);

	zend_declare_property_null(phalconplus_rpc_client_adapter_simple_ce, SL("remoteServerUrl"), ZEND_ACC_PRIVATE);
	zend_declare_property_null(phalconplus_rpc_client_adapter_simple_ce, SL("client"), ZEND_ACC_PRIVATE);
	zend_declare_property_null(phalconplus_rpc_client_adapter_simple_ce, SL("response"), ZEND_ACC_PRIVATE);
	zend_declare_property_string(phalconplus_rpc_client_adapter_simple_ce, SL("formater"), "msgpack", ZEND_ACC_PRIVATE);
	return SUCCESS;
}
...
PHP_METHOD(PhalconPlus_Rpc_Client_Adapter_Simple, __construct)
{
	...
	if (ZEPHIR_IS_EMPTY(&remoteServerUrl)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalconplus_base_exception_ce, "server url can not be empty", "phalconplus/Rpc/Client/Adapter/Simple.zep", 16);
		return;
	}
	ZEPHIR_CALL_FUNCTION(&key, "array_rand", NULL, 155, &remoteServerUrl);
	zephir_check_call_status();
	zephir_array_fetch(&_0, &remoteServerUrl, &key, PH_NOISY | PH_READONLY, "phalconplus/Rpc/Client/Adapter/Simple.zep", 19);
	zephir_update_property_zval(this_ptr, ZEND_STRL("remoteServerUrl"), &_0);
	ZEPHIR_INIT_VAR(&_1);
	object_init_ex(&_1, phalconplus_curl_curl_ce);
	ZEPHIR_CALL_METHOD(NULL, &_1, "__construct", NULL, 156);
	zephir_check_call_status();
...
PHP_METHOD(PhalconPlus_Rpc_Client_Adapter_Simple, callByObject)
{
	...
	zephir_array_fetch_string(&_3, &rawData, SL("service"), PH_NOISY | PH_READONLY, "phalconplus/Rpc/Client/Adapter/Simple.zep", 39);
	ZEPHIR_INIT_VAR(&_4);
