...
PHP_METHOD(PhalconPlus_Assert_Assertion, createException) {
	...
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 20, message, code, propertyPath, value, constraints);
...
PHP_METHOD(PhalconPlus_Assert_Assertion, eq) {
	...
	if (!ZEPHIR_IS_EQUAL(value, value2)) {
		ZEPHIR_CALL_SELF(&v1$$3, "stringify", &_0, 21, value);
		zephir_check_call_status();
		ZEPHIR_CALL_SELF(&v2$$3, "stringify", &_0, 21, value2);
		zephir_check_call_status();
