...
PHP_METHOD(PhalconPlus_Assert_Assertion, createException) {
	...
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", &_0, message, code, propertyPath, value, constraints);
...
PHP_METHOD(PhalconPlus_Assert_Assertion, eq) {
	...
	if (!ZEPHIR_IS_EQUAL(value, value2)) {
		ZEPHIR_CALL_SELF(&v1, "stringify", &_0, value);
		zephir_check_call_status();
		ZEPHIR_CALL_SELF(&v2, "stringify", &_0, value2);
		zephir_check_call_status();
...
PHP_METHOD(PhalconPlus_Assert_Assertion, stringify) {
	...
	zval *value, *_0 = NULL, *val = NULL, *_1 = NULL, _4, _5, *_6;
	...
	ZEPHIR_CALL_FUNCTION(&_1, "is_scalar", &_2, value);
	zephir_check_call_status();
	if (zephir_is_true(_1)) {
		zephir_get_strval(_3, value);
		ZEPHIR_CPY_WRT(val, _3);
		if (zephir_fast_strlen_ev(val) > 100) {
			...
			ZEPHIR_INIT_VAR(_6);
			ZEPHIR_CONCAT_VS(_6, _0, "...");
			ZEPHIR_CPY_WRT(val, _6);
		}
		RETURN_CCTOR(val);
	}
