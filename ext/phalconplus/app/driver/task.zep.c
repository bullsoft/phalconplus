...
ZEPHIR_INIT_CLASS(PhalconPlus_App_Driver_Task) {
	...
	zend_declare_property_bool(phalconplus_app_driver_task_ce, SL("autoHandle"), 1, ZEND_ACC_PROTECTED);
...
PHP_METHOD(PhalconPlus_App_Driver_Task, setHandler) {
	...
	if (EXPECTED(zephir_is_instance_of(handler, SL("PhalconPlus\\App\\Driver\\BaseApplication")))) {
		...
		zephir_throw_exception_debug(&_0$$4, "phalconplus/App/Driver/Task.zep", 41);
		ZEPHIR_MM_RESTORE();
		return;
	}
	RETURN_THIS();
...
PHP_METHOD(PhalconPlus_App_Driver_Task, getHandler) {
	...
	if (UNEXPECTED(ZEPHIR_IS_EMPTY(&_0))) {
		...
		zephir_throw_exception_debug(&_1$$3, "phalconplus/App/Driver/Task.zep", 50);
		ZEPHIR_MM_RESTORE();
		return;
	}
