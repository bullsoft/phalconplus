...
ZEPHIR_INIT_CLASS(PhalconPlus_App_Driver_Srv) {
	...
	zend_declare_property_bool(phalconplus_app_driver_srv_ce, SL("autoHandle"), 1, ZEND_ACC_PROTECTED);
...
PHP_METHOD(PhalconPlus_App_Driver_Srv, __construct) {
	...
	if (UNEXPECTED(zephir_is_true(&_1))) {
		...
		if (!(zephir_instance_of_ev(&backendSrv, phalconplus_rpc_server_abstractserver_ce))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalconplus_base_exception_ce, "Service object(DI[\"backendSrv\"]) must be type of \\PhalconPlus\\Rpc\\Server\\AbstractServer", "phalconplus/App/Driver/Srv.zep", 30);
			return;
		}
	} else {
...
PHP_METHOD(PhalconPlus_App_Driver_Srv, setHandler) {
	...
	zend_bool autoHandle, _0;
	zval *handler, handler_sub, *autoHandle_param = NULL, __$true, __$false;
	zval *this_ptr = getThis();
	...
	zephir_fetch_params_without_memory_grow(1, 1, &handler, &autoHandle_param);
	...
	if (EXPECTED(_0)) {
		...
		ZEPHIR_THROW_EXCEPTION_DEBUG_STRW(phalconplus_base_exception_ce, "Application must be instance of phalcon\\appliction or yar_server", "phalconplus/App/Driver/Srv.zep", 68);
		return;
	}
	RETURN_THISW();
...
PHP_METHOD(PhalconPlus_App_Driver_Srv, getHandler) {
	...
	if (UNEXPECTED(ZEPHIR_IS_EMPTY(&_0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalconplus_base_exception_ce, "Sorry, empty handler", "phalconplus/App/Driver/Srv.zep", 77);
		return;
	}
