...
PHP_METHOD(PhalconPlus_App_Engine_Srv, __construct) {
	...
	if (UNEXPECTED(zephir_is_true(&_0))) {
		...
		if (!(zephir_instance_of_ev(&backendSrv, phalconplus_rpc_server_abstractserver_ce))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalconplus_base_exception_ce, "Service object(DI[\"backendSrv\"]) must be type of PhalconPlus\\Rpc\\Server\\AbstractServer", "phalconplus/App/Engine/Srv.zep", 34);
			return;
		}
	} else {
	...
	zephir_check_call_status();
	zephir_read_property(&_4, &moduleConf, ZEND_STRL("application"), PH_NOISY_CC | PH_READONLY);
	zephir_read_property(&_5, &_4, ZEND_STRL("handler"), PH_NOISY_CC | PH_READONLY);
	if (ZEPHIR_IS_STRING(&_5, "yar")) {
		...
		zephir_check_call_status();
		zephir_update_property_zval(this_ptr, ZEND_STRL("handler"), &_6$$6);
	} else {
		...
		zephir_check_call_status();
		zephir_read_property(&_9$$7, this_ptr, ZEND_STRL("handler"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(PhalconPlus_App_Engine_Srv, setHandler) {
	...
	_0 = (zephir_instance_of_ev(handler, zephir_get_internal_ce(SL("phalcon\\application\\abstractapplication"))));
	if (!(_0)) {
	...
	if (EXPECTED(_0)) {
		...
		ZEPHIR_THROW_EXCEPTION_DEBUG_STRW(phalconplus_base_exception_ce, "Application must be instance of phalcon\\application or yar_server", "phalconplus/App/Engine/Srv.zep", 67);
		return;
	}
	RETURN_THISW();
...
PHP_METHOD(PhalconPlus_App_Engine_Srv, getHandler) {
	...
	if (UNEXPECTED(ZEPHIR_IS_EMPTY(&_0))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalconplus_base_exception_ce, "Sorry, empty srv handler", "phalconplus/App/Engine/Srv.zep", 81);
		return;
	}
