...
ZEPHIR_INIT_CLASS(PhalconPlus_App_Engine_Module) {
	...
	ZEPHIR_REGISTER_CLASS_EX(PhalconPlus\\App\\Engine, Module, phalconplus, app_engine_module, zephir_get_internal_ce(SL("phalcon\\di\\injectable")), phalconplus_app_engine_module_method_entry, 0);

	zend_declare_property_null(phalconplus_app_engine_module_ce, SL("app"), ZEND_ACC_PROTECTED);

	zend_declare_property_null(phalconplus_app_engine_module_ce, SL("handler"), ZEND_ACC_PROTECTED);

	zend_class_implements(phalconplus_app_engine_module_ce, 1, phalconplus_app_engine_appengine_ce);
	return SUCCESS;
...
PHP_METHOD(PhalconPlus_App_Engine_Module, __construct) {
	...
	ZEPHIR_CALL_METHOD(&di, app, "di", NULL, 42);
...
PHP_METHOD(PhalconPlus_App_Engine_Module, setHandler) {
	...
	if (EXPECTED(zephir_instance_of_ev(handler, zephir_get_internal_ce(SL("phalcon\\application"))))) {
		...
		if (zephir_has_constructor(&_0$$4)) {
			...
			ZVAL_STRING(&_2$$4, "Application must be instance of phalcon\\appliction");
			ZEPHIR_CALL_METHOD(NULL, &_0$$4, "__construct", NULL, 0, &_2$$4);
			zephir_check_call_status();
		}
		zephir_throw_exception_debug(&_0$$4, "phalconplus/App/Engine/Module.zep", 46);
		ZEPHIR_MM_RESTORE();
		return;
	}
	RETURN_THIS();
...
PHP_METHOD(PhalconPlus_App_Engine_Module, getHandler) {
	...
	if (UNEXPECTED(ZEPHIR_IS_EMPTY(&_0))) {
		...
		zephir_throw_exception_debug(&_1$$3, "phalconplus/App/Engine/Module.zep", 55);
		ZEPHIR_MM_RESTORE();
		return;
	}
