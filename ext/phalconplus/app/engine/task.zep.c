...
ZEPHIR_INIT_CLASS(PhalconPlus_App_Engine_Task) {
	...
	ZEPHIR_REGISTER_CLASS_EX(PhalconPlus\\App\\Engine, Task, phalconplus, app_engine_task, zephir_get_internal_ce(SL("phalcon\\di\\injectable")), phalconplus_app_engine_task_method_entry, 0);

	zend_declare_property_null(phalconplus_app_engine_task_ce, SL("app"), ZEND_ACC_PROTECTED);

	zend_declare_property_null(phalconplus_app_engine_task_ce, SL("handler"), ZEND_ACC_PROTECTED);

	zend_class_implements(phalconplus_app_engine_task_ce, 1, phalconplus_app_engine_appengine_ce);
	return SUCCESS;
...
PHP_METHOD(PhalconPlus_App_Engine_Task, setHandler) {
	...
	if (EXPECTED(zephir_is_instance_of(handler, SL("PhalconPlus\\App\\Engine\\BaseApplication")))) {
		...
		zephir_throw_exception_debug(&_0$$4, "phalconplus/App/Engine/Task.zep", 44);
		ZEPHIR_MM_RESTORE();
		return;
	}
	RETURN_THIS();
...
PHP_METHOD(PhalconPlus_App_Engine_Task, getHandler) {
	...
	if (UNEXPECTED(ZEPHIR_IS_EMPTY(&_0))) {
		...
		zephir_throw_exception_debug(&_1$$3, "phalconplus/App/Engine/Task.zep", 53);
		ZEPHIR_MM_RESTORE();
		return;
	}
