...
PHP_METHOD(PhalconPlus_Logger_Adapter_FilePlus, __construct) {
	...
	ZEPHIR_CALL_CE_STATIC(NULL, phalconplus_assert_assertion_ce, "notempty", &_0, 33, filePath);
...
PHP_METHOD(PhalconPlus_Logger_Adapter_FilePlus, registerExtension) {
	...
	ZEPHIR_CALL_CE_STATIC(NULL, phalconplus_assert_assertion_ce, "notempty", &_0, 33, ext);
	zephir_check_call_status();
	ZEPHIR_CALL_CE_STATIC(NULL, phalconplus_assert_assertion_ce, "notempty", &_0, 33, types);
	zephir_check_call_status();
	...
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_2, &_1)
		...
		if (zephir_array_isset_string_fetch(&handler, ext2Handler, SS("ext"), 0 TSRMLS_CC)) {
			...
			ZEPHIR_CALL_METHOD(&handler, this_ptr, "open", &_6, 49, _5$$4);
			zephir_check_call_status();
