...
PHP_METHOD(PhalconPlus_Base_SimpleResponse, softClone) {
	...
	if (Z_TYPE_P(&data) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&data), _2, _3, _0)
		{
			...
			ZVAL_COPY(&val, _0);
			ZEPHIR_CALL_METHOD(NULL, this_ptr, "__set", &_4, 80, &key, &val);
...
PHP_METHOD(PhalconPlus_Base_SimpleResponse, setItem) {
	...
	if (!(Z_TYPE_P(key) == IS_NULL)) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "array_push", NULL, 74, &_0$$4, val);
		ZEPHIR_UNREF(&_0$$4);
		zephir_check_call_status();
	}
	RETURN_THIS();
...
PHP_METHOD(PhalconPlus_Base_SimpleResponse, getItem) {
	...
	if (!(Z_TYPE_P(key) == IS_NULL)) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("reset", NULL, 66, &_3$$5);
		ZEPHIR_UNREF(&_3$$5);
		zephir_check_call_status();
		RETURN_MM();
	}
