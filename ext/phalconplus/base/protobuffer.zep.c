...
PHP_METHOD(PhalconPlus_Base_ProtoBuffer, softClone) {
	...
	if (Z_TYPE_P(&data) == IS_ARRAY) {
		...
		while (1) {
			...
			zephir_check_call_status();
				ZEPHIR_CALL_FUNCTION(&_7$$5, "property_exists", &_5, 15, this_ptr, &key);
				zephir_check_call_status();
...
PHP_METHOD(PhalconPlus_Base_ProtoBuffer, __set) {
	...
	if ((zephir_method_exists(this_ptr, &method)  == SUCCESS)) {
		...
		ZVAL_LONG(&_3$$3, 0);
		ZEPHIR_CALL_METHOD(NULL, &param, "__construct", NULL, 16, &_2$$3, &_3$$3);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&_4$$3, &param, "getclass", NULL, 17);
		zephir_check_call_status();
		if (zephir_is_true(&_4$$3)) {
			ZEPHIR_CALL_METHOD(&_5$$4, &param, "getclass", NULL, 17);
			zephir_check_call_status();
			...
			ZEPHIR_CALL_METHOD(&_6$$4, &paramClassRef, "issubclassof", NULL, 18, &_7$$4);
			zephir_check_call_status();
			if (zephir_is_true(&_6$$4)) {
				ZEPHIR_CALL_METHOD(&paramObj, &paramClassRef, "newinstance", NULL, 19);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(NULL, &paramObj, "softclone", NULL, 0, val);
				zephir_check_call_status();
				ZEPHIR_CPY_WRT(val, &paramObj);
			} else {
				ZEPHIR_CALL_METHOD(&paramObj, &paramClassRef, "newinstance", NULL, 19, val);
				zephir_check_call_status();
				ZEPHIR_CPY_WRT(val, &paramObj);
			}
		}
	...
	if (_9) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_11$$9, "__construct", NULL, 20, &_12$$9);
		zephir_check_call_status();
...
PHP_METHOD(PhalconPlus_Base_ProtoBuffer, getSelfVars) {
	...
	ZEPHIR_CALL_METHOD(NULL, &objReflection, "__construct", NULL, 21, this_ptr);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&pros, &objReflection, "getproperties", NULL, 22);
	zephir_check_call_status();
...
PHP_METHOD(PhalconPlus_Base_ProtoBuffer, toArray) {
	...
	if (Z_TYPE_P(&pros) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&pros), _2, _3, _0)
		{
			...
			if (_4$$5) {
				ZEPHIR_CALL_METHOD(&_7$$6, this_ptr, "toarray", &_8, 23, &val);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_11$$10) {
					ZEPHIR_CALL_METHOD(&_14$$11, this_ptr, "toarray", &_8, 23, &val);
					zephir_check_call_status();
