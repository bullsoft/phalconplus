...
PHP_METHOD(PhalconPlus_Db_UnitOfWork, exec)
{
	...
	ZEPHIR_CALL_METHOD(NULL, &objects, "addall", NULL, 114, &_1);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, &objects, "rewind", NULL, 115);
	zephir_check_call_status();

	/* try_start_1: */

		while (1) {
			ZEPHIR_CALL_METHOD(&_2$$3, &objects, "valid", &_3, 116);
			zephir_check_call_status_or_jump(try_end_1);
			if (!(zephir_is_true(&_2$$3))) {
				break;
			}
			ZEPHIR_CALL_METHOD(&obj, &objects, "current", &_4, 117);
			zephir_check_call_status_or_jump(try_end_1);
			...
			if (zephir_is_instance_of(&obj, SL("Phalcon\\Mvc\\Model"))) {
				...
				ZEPHIR_CALL_FUNCTION(NULL, "iterator_apply", &_8, 119, &obj, &_6$$6, &_7$$6);
				zephir_check_call_status_or_jump(try_end_1);
			}
			...
			ZEPHIR_CALL_METHOD(NULL, &objects, "next", &_14, 120);
			zephir_check_call_status_or_jump(try_end_1);
		}
	...
	if (EG(exception)) {
		...
		if (zephir_is_instance_of(&_15, SL("Phalcon\\Mvc\\Model\\Transaction\\Failed"))) {
			...
			ZEPHIR_CALL_METHOD(NULL, &objects, "removeall", NULL, 121, &_16$$8);
			zephir_check_call_status();
			RETURN_MM_BOOL(0);
		}
	}
	zephir_read_property(&_17, this_ptr, ZEND_STRL("objects"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(NULL, &objects, "removeall", NULL, 121, &_17);
	zephir_check_call_status();
	RETURN_MM_BOOL(1);
}
