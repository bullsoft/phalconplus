...
PHP_METHOD(PhalconPlus_Db_UnitOfWork, exec) {
	...
	ZEPHIR_CALL_METHOD(NULL, &objects, "addall", NULL, 99, &_1);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, &objects, "rewind", NULL, 100);
	zephir_check_call_status();

	/* try_start_1: */

		while (1) {
			ZEPHIR_CALL_METHOD(&_2$$3, &objects, "valid", &_3, 101);
			zephir_check_call_status_or_jump(try_end_1);
			if (!(zephir_is_true(&_2$$3))) {
				break;
			}
			ZEPHIR_CALL_METHOD(&obj, &objects, "current", &_4, 102);
			zephir_check_call_status_or_jump(try_end_1);
			ZEPHIR_CALL_METHOD(&info, &objects, "getinfo", &_5, 103);
			zephir_check_call_status_or_jump(try_end_1);
			...
			if (zephir_instance_of_ev(&obj, zephir_get_internal_ce(SL("phalcon\\mvc\\model")))) {
				...
				ZEPHIR_CALL_FUNCTION(NULL, "iterator_apply", &_8, 104, &obj, &_6$$6, &_7$$6);
				zephir_check_call_status_or_jump(try_end_1);
			}
			...
			ZEPHIR_CALL_METHOD(NULL, &objects, "next", &_14, 105);
			zephir_check_call_status_or_jump(try_end_1);
		}
	...
	if (EG(exception)) {
		...
		if (zephir_instance_of_ev(&_15, zephir_get_internal_ce(SL("phalcon\\mvc\\model\\transaction\\failed")))) {
			...
			ZEPHIR_CALL_METHOD(NULL, &objects, "removeall", NULL, 106, &_16$$8);
			zephir_check_call_status();
			RETURN_MM_BOOL(0);
		}
	}
	zephir_read_property(&_17, this_ptr, SL("objects"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(NULL, &objects, "removeall", NULL, 106, &_17);
	zephir_check_call_status();
	RETURN_MM_BOOL(1);
