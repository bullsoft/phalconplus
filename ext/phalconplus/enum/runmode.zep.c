...
PHP_METHOD(PhalconPlus_Enum_RunMode, getScriptPath) {
	...
	zval script, _0, _1, _2, _3, _4, _5$$3;
	...
	ZEPHIR_INIT_VAR(&_0);
	ZEPHIR_GET_CONSTANT(&_0, "APP_ROOT_COMMON_LOAD_DIR");
	zephir_read_property(&_1, this_ptr, SL("scripts"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_OBS_VAR(&_3);
	zephir_read_property(&_3, this_ptr, SL("val"), PH_NOISY_CC);
	zephir_array_fetch(&_2, &_1, &_3, PH_NOISY | PH_READONLY, "phalconplus/Enum/RunMode.zep", 34 TSRMLS_CC);
	ZEPHIR_INIT_VAR(&script);
	ZEPHIR_CONCAT_VV(&script, &_0, &_2);
	ZEPHIR_CALL_FUNCTION(&_4, "is_file", NULL, 41, &script);
	zephir_check_call_status();
	if (!(zephir_is_true(&_4))) {
		ZEPHIR_INIT_VAR(&_5$$3);
		ZEPHIR_CONCAT_SV(&_5$$3, "PHP Notice:  PhalconPlus\\Bootstrap Global load file not exists: ", &script);
		ZEPHIR_CALL_FUNCTION(NULL, "error_log", NULL, 50, &_5$$3);
		zephir_check_call_status();
	}
	RETURN_CCTOR(&script);
...
PHP_METHOD(PhalconPlus_Enum_RunMode, getMapClassName) {
	...
	zephir_array_fetch(&_1, &_0, &_2, PH_NOISY | PH_READONLY, "phalconplus/Enum/RunMode.zep", 43 TSRMLS_CC);
	RETURN_CTOR(&_1);
...
zend_object *zephir_init_properties_PhalconPlus_Enum_RunMode(zend_class_entry *class_type TSRMLS_DC) {
	...
	
	{
		...
		if (Z_TYPE_P(&_0) == IS_NULL) {
			...
			add_assoc_stringl_ex(&_1$$3, SL("Micro"), SL("default-web.php"));
