...
class Local extends AbstractClient
{
    ...
    private function callByParams(string! service, string! method, <ProtoBuffer> request) -> <ProtoBuffer>
    {
        ...
        if !class_exists(serviceClass) {
            throw new \PhalconPlus\Base\Exception("Service class not exists: " . serviceClass);
        }
        ...
        if is_callable([serviceObj, method]) {
            ...
            if is_object(request) && !(request instanceof ProtoBuffer) {
                throw new \PhalconPlus\Base\Exception("Your input is not allowed. Request: ". get_class(request));
            ...
            throw new \PhalconPlus\Base\Exception("No service found: " . serviceClass . "::" . method);
        }
    }
