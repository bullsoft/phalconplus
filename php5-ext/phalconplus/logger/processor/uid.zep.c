...
PHP_METHOD(PhalconPlus_Logger_Processor_Uid, __construct) {
	...
	ZEPHIR_SINIT_VAR(_0);
	ZVAL_STRING(&_0, "", 0);
	ZEPHIR_CALL_FUNCTION(&_1, "uniqid", NULL, 73, &_0, ZEPHIR_GLOBAL(global_true));
	zephir_check_call_status();
	ZEPHIR_SINIT_NVAR(_0);
	ZVAL_STRING(&_0, "md5", 0);
	ZEPHIR_CALL_FUNCTION(&_2, "hash", NULL, 74, &_0, _1);
	zephir_check_call_status();
	ZEPHIR_SINIT_NVAR(_0);
	ZVAL_LONG(&_0, 0);
	ZEPHIR_INIT_VAR(_3);
	zephir_substr(_3, _2, 0 , zephir_get_intval(len), 0);
	zephir_update_property_this(getThis(), SL("uid"), _3 TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
